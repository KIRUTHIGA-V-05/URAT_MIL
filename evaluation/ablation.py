...
Produces a summary table and bar chart comparing AUC / ECE / Acc across variants.
...
def run_variant(variant: dict, stage: int, python: str = sys.executable):
    """Launch training subprocess for one ablation variant."""
...
def run_eval(ckpt: str, variant: dict, stage: int, python: str = sys.executable) -> dict:
    """Launch evaluation subprocess and load results."""
    run_name = f"{variant['name']}_s{stage}"
...
def plot_ablation_comparison(results_list: list, out_dir: str, stage: int):
    """
    Bar chart comparing AUC, Accuracy, ECE across ablation variants.
    Paper: Table showing contribution of each module.
    """
...
def print_table(results_list: list):
    """Print a clean ASCII comparison table."""
