...
Paper contributions per module:
  Module 1: Foundation encoder (ResNet18 / Swin) — feature_dim → latent_dim projection
  ...
  Module 5: Uncertainty-aware MIL attention       — Eq. 13–15
"""
...
class OODModule(nn.Module):
    ...
    def ot_score(self, z: torch.Tensor) -> torch.Tensor:
        """Eq. 11: min cosine distance to prototype set."""
    ...
    # ── Tier-2 ─────────────────────────────────────────────
    def recon_score(self, h: torch.Tensor, z: torch.Tensor) -> torch.Tensor:
    ...
    def forward(self, h: torch.Tensor, z: torch.Tensor):
        ...
        gate = (s_ot <= self.delta_artifact) & (s_ood <= self.delta_near)  # (N,) bool
    ...
    def forward(self, z, u_total, gate_mask):
        ...
        for k in range(self.n_heads):
            ...
            if self.use_unc_gate:
                a = a / u_safe                    # Eq. 13 denominator weight
...
class URATMIL(nn.Module):
    ...
    def forward(self, feats: torch.Tensor, src_feats=None, tgt_feats=None):
        """
        feats      : (N, feature_dim) — one bag
        src_feats  : (M, feature_dim) — source domain batch for alignment
        tgt_feats  : (K, feature_dim) — target domain batch for alignment

        Returns dict with all intermediate tensors and scalar losses.
        """
        device = feats.device
    ...
    def compute_loss(self, out, label, lambda_kl, lambda_align, lambda_recon, lambda_cal):
        ...
        l_ce  = F.cross_entropy(logit_t, label_t)

        # Calibration signal: push confidence toward accuracy
